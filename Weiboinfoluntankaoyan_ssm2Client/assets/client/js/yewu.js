...
function toMyBlog(uid){
    ...
    ajaxCallback("listJ",{table:"blog",btype:2,uid:userinfo.id},function (list) {
...
function toTurlPage(){
    changePage("turlpage");
    var ltpl = {};
    ltpl.tpl = '<li onclick="toframe(\'%s\');">'+
        '<h2>%s</h2>'+
        '</li>';
    ltpl.colums = ["turl","title"];
    $("#turllist").data("property",JSON.stringify(ltpl));
    ajaxCallback("listJ",{table:"turl"},function (listdata) {
        focuslist = listdata;
        $("#turllist").refreshShowListView(listdata);
    })
}
